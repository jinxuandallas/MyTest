...
func _process(delta):
	if in_edge:
		...
		if mouse_pos.x>screen_size.x-margin_container.get_constant("margin_right")&&camera.offset.x<2000:
			camera.offset.x+=delta*SPEED/zoom
			
		if mouse_pos.x<margin_container.get_constant("margin_left")&&camera.offset.x>-200:
			camera.offset.x-=delta*SPEED/zoom

		if mouse_pos.y<margin_container.get_constant("margin_top"):
			camera.offset.y-=delta*SPEED/zoom
